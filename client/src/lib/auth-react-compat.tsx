...
export function useSession() {
  ...
  // Provide an update() compatible with NextAuth's useSession().update()
  const update = async () => {
    // Trigger a refetch of the session from the server
    await authClient.getSession();
  };

  return { data: session, status, error, update } as const;
}

// NextAuth.signIn(provider, options) → better-auth signIn
type CompatSignInResult = {
  ok: boolean;
  error: string | null;
  status: number;
  url: string | null;
...
export async function getProviders(): Promise<Record<string, { id: string; name: string }> | null> {
  try {
    const res = await apiFetch("/api/v1/auth/providers-list", { cache: "no-store" });
