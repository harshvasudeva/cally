...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            apiFetch("/api/v1/user/accounts").then(res => res.ok ? res.json() : []),
            apiFetch("/api/v1/user/profile").then(res => res.ok ? res.json() : null),
            apiFetch("/api/v1/user/ical-feed").then(res => res.ok ? res.json() : null).catch(() => null),
            apiFetch("/api/v1/holidays/countries").then(res => res.ok ? res.json() : []).catch(() => []),
    ...
    const generateIcalFeed = useCallback(async () => {
        setIcalLoading(true)
        try {
            const res = await apiFetch("/api/v1/user/ical-feed", { method: "POST" })
            if (res.ok) {
    ...
    const revokeIcalFeed = useCallback(async () => {
        setIcalLoading(true)
        try {
            const res = await apiFetch("/api/v1/user/ical-feed", { method: "DELETE" })
