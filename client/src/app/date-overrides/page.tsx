...
export default function DateOverridesPage() {
    ...
    const fetchOverrides = async () => {
        try {
            const res = await apiFetch("/api/v1/date-overrides")
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await apiFetch("/api/v1/date-overrides", {
                method: "POST",
    ...
    const handleDelete = async (id: string) => {
        setDeleting(id)
        try {
            await apiFetch(`/api/v1/date-overrides/${id}`, { method: "DELETE" })
            fetchOverrides()
        } catch (error) {
    ...
    const fetchHolidays = useCallback(async (countryCode?: string) => {
        ...
        try {
            const res = await apiFetch(`/api/v1/holidays?country=${cc}&year=${holidayYear}`)
            if (res.ok) {
                const data = await res.json()
