...
export default function SettingsPage() {
    ...
    const handleTestEmail = async () => {
        ...
        setFeedback(null)
        try {
            const res = await apiFetch("/api/v1/admin/test-email", { method: "POST" })
            const data = await res.json()
            if (res.ok) {
    ...
    const handleUploadCerts = async () => {
        ...
        try {
            ...
            const res = await apiFetch("/api/v1/admin/domain", { method: "POST", body: formData })
            const data = await res.json()
            if (res.ok) {
    ...
    const handleSaveDomain = async () => {
        ...
        setFeedback(null)
        try {
            const res = await apiFetch("/api/v1/admin/domain", {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        type="number"
                                        value={settings.maxLoginAttempts || 0}
                                ...
                                <div>
                                    ...
                                    <input
                                        type="number"
                                        value={settings.lockoutDuration || 0}
