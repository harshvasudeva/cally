...
export interface LegacySelectProps extends Omit<React.SelectHTMLAttributes<HTMLSelectElement>, "children"> {
  label?: string
  error?: string
  helpText?: string
  ...
  ({ label, error, helpText, options, placeholder, prefixIcon, className = "", id, ...props }, ref) => {
    ...
    return (
      <div className="w-full">
        {label && (
          <label htmlFor={selectId} className="block text-[13px] font-medium text-muted-foreground mb-1.5">
            {label}
          </label>
        )}
...
LegacySelect.displayName = "Select"
