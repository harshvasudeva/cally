...
export default function CalendarView({
    ...
    const fetchEvents = useCallback(async () => {
        try {
            const [eventsRes, appointmentsRes, overridesRes, streamEventsRes] = await Promise.all([
                apiFetch("/api/v1/events"),
                apiFetch("/api/v1/appointments"),
                apiFetch("/api/v1/date-overrides"),
                apiFetch("/api/v1/stream-events"),
            ])
            ...
            const streamEventsData: { id: string; title: string; start: string; end: string; allDay: boolean; color: string; streamName: string }[] = streamEventsRes.ok ? await streamEventsRes.json() : []
            ...
            const calendarEvents: CalendarEvent[] = [
                ...
                // Stream events (read-only overlays from external .ics feeds)
                ...streamEventsData.map((se) => ({
                    id: se.id,
                    title: se.title,
                    start: se.start,
                    end: se.end,
    ...
    const handleEventClick = useCallback((info: any) => {
        // Don't open edit modal for date overrides / holidays / stream events
        if (info.event.extendedProps?.type === "date-override") return
        if (info.event.extendedProps?.type === "stream") return
    ...
    const handleEventDrop = useCallback(async (info: any) => {
        ...
        try {
            await apiFetch(`/api/v1/events/${info.event.id}`, {
                method: "PUT",
