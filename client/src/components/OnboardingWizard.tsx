...
export default function OnboardingWizard({ onComplete }: OnboardingWizardProps) {
    ...
    const handleNext = async () => {
        ...
        if (step === 2) {
            ...
            try {
                await apiFetch("/api/v1/availability", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
        ...
        if (step === 3) {
            try {
                await apiFetch("/api/v1/appointment-types", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        name: eventName,
    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            if (res.ok) {
                // Hard reload to clear better-auth's cookie cache so the
                // calendar page sees the updated onboardingCompleted flag.
